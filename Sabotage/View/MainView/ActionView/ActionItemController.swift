...
class ActionItemController: UIViewController {
    // 이 컨트롤러에서 데이터를 추가하거나 수정하는 UI를 구현합니다.
    // 예를 들어, 텍스트 필드와 버튼을 사용하여 데이터를 입력받고 Realm에 저장할 수 있습니다.
    ...
    // 예시: 버튼을 눌렀을 때 데이터를 추가하는 함수
    @objc func addButtonTapped() {
        let realm = try! Realm()
        let newItem = CategoryItem()
        newItem.categoryType = "새 카테고리"
        newItem.content = "새 내용"
        newItem.categoryImageName = "category_new.png"
